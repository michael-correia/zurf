...
#[test]
fn test_lr_deserialize_with_extension() {
    let mut data = vec![
        0xFD, 0xD0, 0x9B, 0xC7, // Home ID
        0x00, 0x11, 0x00, // SRC = 1, DST = 0x100
        26,   // mpdu_len_size (14 + 1 control + 3 extension data + 8 payload)
        0x41, // Frame Control (Singlecast | has_extension)
        0x02, // Sequence number (2)
        0xA4, // Noise floor (-92)
        0xFA, // Tx power (-6)
        0x03, // Extension control (type 0, length 3)
        0xAA, 0xBB, 0xCC, // Extension data
        0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, // Payload
    ];
    let crc = Data::crc_ccitt(&data);
    data.extend_from_slice(&crc.to_be_bytes());
